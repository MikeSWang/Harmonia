...
def test_init(discrete_spectrum, spherical_array):
    ...
    assert all([
        ...
        for block, alternative_block in zip(
            spherical_array.init_array, TEST_DATA_ARRAY
            )
        ])
...
def test_build(discrete_spectrum, spherical_array):
    ...
    assert all([
        np.all(block == alternative_block)
        for block, alternative_block in zip(
            spherical_array.init_array,
            alternative_spherical_array.init_array
            )
        ])
    assert all([
        np.all(block == alternative_block)
        for block, alternative_block in zip(
            spherical_array.init_indices,
            alternative_spherical_array.init_indices
            )
        ])
...
@pytest.mark.parametrize(
    ...
    [('natural',
      ...
      [(0, 0, 1), (0, 0, 2), (0, 0, 3), (1, -1, 1), (1, -1, 2), (1, 0, 1),
       (1, 0, 2), (1, 1, 1), (1, 1, 2),]
      ),
     ('lnm',
      [3.14, 6.28, 9.42, 4.49, 4.49, 4.49, 7.73, 7.73, 7.73,],
      [(0, 0, 1), (0, 0, 2), (0, 0, 3), (1, -1, 1), (1, 0, 1), (1, 1, 1),
       (1, -1, 2), (1, 0, 2), (1, 1, 2),]
      ),
     ('ln',
      [3.14, 6.28, 9.42, 4.49, 7.73, 5.76, 9.1, 6.99, 8.18,],
      [(0, 1), (0, 2), (0, 3), (1, 1), (1, 2), (2, 1), (2, 2), (3, 1), (4, 1),]
      ),
     ('k',
      [3.14, 4.49, 4.49, 4.49, 5.76, 5.76, 5.76, 5.76, 5.76,],
      [(0, 0, 1), (1, -1, 1), (1, 0, 1), (1, 1, 1), (2, -2, 1), (2, -1, 1),
       (2, 0, 1), (2, 2, 1), (2, 1, 1),]
      ),
     ('scale',
      [3.14, 4.49, 5.76, 6.28, 6.99, 7.73, 8.18, 9.1, 9.36,],
      [(0, 1), (1, 1), (2, 1), (0, 2), (3, 1), (1, 2), (4, 1), (2, 2), (5, 1),]
      )]
    )
        ...
        [out_subarr == morphed_subarr
         for out_subarr, morphed_subarr in zip(
             ...
             spherical_array.morph(
                 in_array, in_struct, out_struct, 'data'
                 )
             )
         ]
        )
    assert all(
        [out_subarr == morphed_subarr
         for out_subarr, morphed_subarr in zip(
             out_indices,
             spherical_array.morph(
                 in_indices, in_struct, out_struct, 'index'
                 )
             )
         ]
        )
